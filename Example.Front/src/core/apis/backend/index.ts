...
import { BackendApi as _BackendApi, Configuration, Todo } from "./generated";
...
export class BackendApi {
	...
	constructor() {
		...
		this.todo = {
			common: {
				getAll: async () => (await todoApi.getAll()).data,
				add: async (label: string) => (await todoApi.add(label)).data,
				check: async (id: string) => (await todoApi.check(id)).data,
				delete: async (id: string) => {
					await todoApi._delete(id);
				},
			},
			user: {
				getAll: async () => (await todoApi.getAllForUser()).data,
				add: async (label: string) => (await todoApi.addForUser(label)).data,
				check: async (id: string) => (await todoApi.checkForUser(id)).data,
				delete: async (id: string) => {
